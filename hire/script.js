    ...
    btn.addEventListener("click", () => {
      ...
      shareModal.querySelectorAll(".share-option").forEach(option => {
        option.onclick = () => {
          ...
          const shareText = `${project.title} - ${project.description || "Check this out"}`;
          ...
          switch (platform) {
            ...
            case "whatsapp":
              ...
              break;
            case "copy":
              navigator.clipboard.writeText(projectURL).then(() => alert("Copied to clipboard"));
              break;
            case "markdown":
              navigator.clipboard.writeText(`[${project.title}](${projectURL})`).then(() => alert("Markdown copied"));
              break;
            case "devto":
              const devContent = `---\ntitle: ${project.title}\ndescription: ${project.description}\n---\n\nCheck it out 👉 ${projectURL}`;
              navigator.clipboard.writeText(devContent).then(() => alert("Dev.to blog draft copied!"));
              break;
          }
